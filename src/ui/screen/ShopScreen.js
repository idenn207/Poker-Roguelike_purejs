...
class ShopScreen extends UICore {
  ...
  createShopScreen() {
    ...
    this._draw.setHTML(goldStat, '<span class="shop-stat-label">💰 GOLD</span><span class="shop-stat-value" id="shopPlayerGold">250</span>');
  ...
  #createTrumpCard(data, index) {
    ...
    // Tooltip
    const tooltip = this._draw.createElement('div', 'product-tooltip');
    const tooltipTitle = this._draw.createElement('div', 'tooltip-title');
    this._draw.setText(tooltipTitle, data.title);
    const tooltipDesc = this._draw.createElement('div', 'tooltip-description');
    this._draw.setText(tooltipDesc, data.desc);
    this._draw.appendChild(tooltip, tooltipTitle);
    this._draw.appendChild(tooltip, tooltipDesc);
    ...
    this._draw.appendChild(card, tooltip);
  ...
  #filterShopProducts(category) {
    ...
    allProducts.forEach((product, index) => {
      ...
      if (productCategory === category) {
        ...
        // 순차 애니메이션
        this._draw.addStyle(product, { animation: 'none' });
        setTimeout(() => {
          this._draw.addStyle(product, { animation: `fadeInUp 0.3s ease ${index * 0.05}s forwards` });
        }, 10);
      } else {
  ...
  /**
   * 아이템/업그레이드 카드 생성
   * @param {Object} itemData - 아이템 데이터 (icon, title, desc, price, category, type)
   * @returns {HTMLElement} 카드 엘리먼트
   */
  #createItemCard(itemData) {
    const card = this._draw.createElement('div', `shop-product-card ${itemData.type}-card`);
    this._draw.addDataset(card, 'category', itemData.category);

    // 아이콘
    const icon = this._draw.createElement('div', `${itemData.type}-icon`);
    this._draw.setText(icon, itemData.icon);

    // 가격
    const price = this._draw.createElement('div', 'product-price');
    this._draw.setText(price, `💰 ${itemData.price}`);

    // Tooltip
    const tooltip = this._draw.createElement('div', 'product-tooltip');

    const tooltipTitle = this._draw.createElement('div', 'tooltip-title');
    this._draw.setText(tooltipTitle, itemData.title);

    const tooltipDesc = this._draw.createElement('div', 'tooltip-description');
    this._draw.setText(tooltipDesc, itemData.desc);
